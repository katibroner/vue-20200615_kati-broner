...
const { mockMeetups } = require('./mock-meetups');
require('./mock-date');
...
describe('2-module-3-task', () => {
  describe('MeetupsCalendar', () => {
    it('MeetupsCalendar должен иметь обязательный пропс meetups по списком митапов', () => {
    ...
    it('MeetupsCalendar должен вывести 5 строк по 7 недель для апреля 2020', () => {
      Date.mockNow(new Date('2020-04-15'));
      const wrapper = shallowMount(MeetupsCalendar, {
        propsData: { meetups: [] },
      });
      const cells = wrapper.findAll('.rangepicker__cell').wrappers;
    ...
    it('MeetupsCalendar должен вывести май 2020', () => {
      Date.mockNow(new Date('2020-05-15'));
      const wrapper = shallowMount(MeetupsCalendar, {
        propsData: { meetups: [] },
      });
      testMonthCalendar(
    ...
    it('MeetupsCalendar должен вывести июнь 2020', () => {
      Date.mockNow(new Date('2020-06-15'));
      const wrapper = shallowMount(MeetupsCalendar, {
        propsData: { meetups: [] },
      });
    ...
    it('MeetupsCalendar должен показать февраль 2021 после 9 кликов на след. месяц с 31 мая 2020', async () => {
      Date.mockNow(new Date('2020-05-31'));
      const wrapper = shallowMount(MeetupsCalendar, {
        propsData: { meetups: [] },
      });
      for (let i = 0; i < 9; i++) {
    ...
    it('MeetupsCalendar должен показать митапы 12 мая после перехода на следующий месяц с апреля 2020', async () => {
      ...
      expect(events).toEqual([mockMeetups[4].title, mockMeetups[5].title]);
    });
  });
});
